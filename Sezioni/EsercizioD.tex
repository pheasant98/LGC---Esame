...
\begin{itemize}
	...
	$\dfrac{P \overset{\alpha_{1}}\rightarrow P'}{P|R \overset{\alpha_{1}}\rightarrow P'\overset{t'}\rightarrow}$ \textit{PAR-L} \\
	
	t = $\alpha_{1}.t'$ con |t'| = n, per ipotesi induttiva t' $\in \tr{P'|R} \Rightarrow t' \in \tr{Q'|R} $, \\
	si applica il lemma:\\
	...
	$\dfrac{R \overset{\alpha_{1}}\rightarrow R'}{P|R \overset{\alpha_{1}}\rightarrow R'\overset{t'}\rightarrow}$ \textit{PAR-R} \\
	
		t = $\alpha_{1}.t'$ con |t'| = n, per ipotesi induttiva t' $\in \tr{P|R'} \Rightarrow t' \in \tr{Q|R'} $, e quindi 
		t $\in $\tr{$\alpha_{1}$.(Q|R')}. \\
	...
	t = $\tau_{1}.t'$ con |t'| = n, per ipotesi induttiva t' $\in \tr{P'|R'} \Rightarrow t' \in \tr{Q'|R'} $, \\
	si applica il lemma:\\
	\tr{P'} $\subseteq$ \tr{$\displaystyle\sum_{}^{} \{Q'|Q \overset{\alpha_{1}}\rightarrow Q'\}$} e quindi t' $\in$ \tr{P'|R'} $\Rightarrow$ t' $\in$ \tr{$\displaystyle\sum_{Q \overset{\alpha_{1}}\rightarrow Q'}^{} (Q'|R') $}
...
\begin{itemize}
	...
	$\dfrac{Q \overset{\alpha_{1}}\rightarrow Q'}{Q|R \overset{\alpha_{1}}\rightarrow Q'\overset{t'}\rightarrow}$ \textit{PAR-L} \\
	
	t = $\alpha_{1}.t'$ con |t'| = n, per ipotesi induttiva t' $\in \tr{Q'|R} \Rightarrow t' \in \tr{P'|R} $, \\
	si applica il lemma:\\
	...
	$\dfrac{R \overset{\alpha_{1}}\rightarrow R'}{Q|R \overset{\alpha_{1}}\rightarrow R'\overset{t'}\rightarrow}$ \textit{PAR-R} \\
	
	t = $\alpha_{1}.t'$ con |t'| = n, per ipotesi induttiva t' $\in \tr{Q|R'} \Rightarrow t' \in \tr{P|R'} $, e quindi 
	t $\in $\tr{$\alpha_{1}$.(P|R')}. \\
...
Il caso del contesto restrizione L sul processo P ha la seguente uguaglianza: \tr{P\textbackslash L} = \tr{P}\textbackslash\{t=...$\alpha_{x}$...|$\alpha_{x}\in$L\} cioè le traccie che stanno in \tr{P} non ci sono nell'insieme di restrizione definito precedentemente. Se questo è vero, dato che per ipotesi induttiva \tr{P} = \tr{Q} e quindi \tr{Q\textbackslash L} = \tr{Q}\textbackslash \{t=...$\alpha_{x}$...|$\alpha_{x}\in$L\}, allora \tr{P\textbackslash L} = \tr{Q\textbackslash L}. 
...
t ha una prima interazione seguita poi dalla traccia t', quindi P\textbackslash L $ \overset{\alpha_{1}}\rightarrow $ X' $\overset{t'}\rightarrow$ ovvero viene applicata una transizione secondo la regola della restrizione arrivando in un certo processo X', quindi:
	...
	$\alpha_{1}.t' \in \alpha_{1}$.\tr{P'\textbackslash L}$\subseteq$ \tr{P\textbackslash L} con t $\not \in$ \{t=...$\alpha_{x}$...|$\alpha_{x}\in$L\}  allora t $\in$\tr{P}\textbackslash \{t=...$\alpha_{x}$...|$\alpha_{x}\in$L\}.
...
t = $\alpha_{1}.t'$ con |t'| = n, per ipotesi induttiva t' $\in$ \tr{P'}$\textbackslash$ \{t=...$\alpha_{x}$...|$\alpha_{x}\in$L\}, quindi 
$\alpha_{1}.t' \in \alpha_{1}$.\tr{P'}\textbackslash \{t=...$\alpha_{x}$...|$\alpha_{x}\in$L\} $\subseteq$ \tr{P}\textbackslash \{t=...$\alpha_{x}$...|$\alpha_{x}\in$L\} con t $\not \in$ \{t=...$\alpha_{x}$...|$\alpha_{x}\in$L\}  allora t $\in$\tr{P\textbackslash L}.
\\
